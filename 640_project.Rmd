...
sample_splitting <- function(df,k = 2, ml_method = "random forest", seed=123){
  ...
  for (i in 1:k){
    ...
    if(ml_method == "random forest"){
      ...
      fit_y <- train(Y~., data=train, method="rf", tuneGrid=tunegrid, trControl=control)
      fit_z <- train(D~., data=train, method="rf", tuneGrid=tunegrid, trControl=control)
    ...
    if(ml_method == "svm"){
      ...
      fit_y =train(Y ~., data = train, method ="svmRadial", trControl = control,  preProcess = c("center","scale"))
      fit_z = train(D ~., data = train, method = "svmRadial", trControl = control,  preProcess = c("center","scale"))
       # predict Y and Z
      py <- predict(fit_y, prediction)
      pz <- predict(fit_z, prediction)
    ...
    if(ml_method == "xgboost"){
      ...
      fit_y <- train(data = dtrain, label = train$Y, max.depth = 2, eta = 1, nthread = 2, nrounds = 2, objective = "reg:squarederror", verbose = FALSE)
...
full_sample <- function(df, ml_method = "random forest", seed=123){
  ...
  if(ml_method == "random forest"){
      ...
      tunegrid <- expand.grid(.mtry=c(1:15))
      fit_y <- train(Y~., data=df, method="rf", tuneGrid=tunegrid, trControl=control)
      fit_z <- train(D~., data=df, method="rf", tuneGrid=tunegrid, trControl=control)
  ...
  if(ml_method == "svm"){
      ...
      fit_y = train(Y ~., data = df, method = "svmRadial", trControl = control,  preProcess = c("center","scale"))
      fit_z = train(D ~., data = df, method = "svmRadial", trControl = control,  preProcess = c("center","scale"))
...
for(i in 1:100){
  ...
  ss <- c(ss,sample_splitting(df1,seed=640,  ml_method = "svm"))
  fs <- c(fs,full_sample(df1,seed=640,  ml_method = 'svm'))
}
