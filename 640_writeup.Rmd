...
- For $k=1$, using machine learning methods to predict $Y$ and $D$ using $X$ by $\widehat{\mathbb{E}[Y|X])}$ and $\widehat{\mathbb{E}[D|X])}$ respectively on $I_{k=1}$.  
- Get residuals $\hat{U}=Y-\widehat{\mathbb{E}[Y|X])}$ and $\hat{V}=D-\widehat{\mathbb{E}[D|X])}$  
- Regress $\hat{U}$ on $\hat{V}$ to get the estimate $\widehat{\theta_{0,k=1}}$  
