...
For the Table 3, we compared the effect of sample splitting under different combinations of sample size the number of covariates. We fix the variance for the error term be 0.01, coefficients for covariates follow normal distribution, and the outcome model be generated from linear functions

```{r,echo=F,warning=F,message=F}
idx3 = c(1,3,5,7,65,67,69,71)
table3 = make_table2(idx3)
table3 %>% flextable() %>% font(fontname = "Cambria",part = "all") %>%
  set_header_labels(method="Method",fll="Full",ll="Twofold",flh="Full",lh="Twofold",fhl="Full",hl="Twofold",fhh="Full",hh="Twofold") %>%
  add_header_row(top = TRUE, values = c("","N=50, Ncov=5", "N=50, Ncov=100", "N=1000, Ncov=5","N=1000, Ncov=100"), 
                 colwidths = c(1,2,2,2,2)) %>% autofit() %>%
